...
def warp_image_to_polygon(img_arry, polygon, coordinate_array):
    ...
    try:
        ...
        georef_image_array = skimage.transform.warp(img_arry_equalized, np.array(h_matrix),
                                                    output_shape=(img_arry_equalized.shape[0],
                                                                  img_arry_equalized.shape[1]),
                                                    )
    except Exception as e:
        print(f"Error warping image to polygon: {e}")
        return None
...
def gps_to_pixel(gps_coord, x_min, y_max, resolution_x, resolution_y):
    ...
    try:
        ...
        print(f"Error converting GPS to pixel: {e}")
...
def array2ds(cv2_array, polygon_wkt):
    ...
    if not isinstance(cv2_array, np.ndarray):
        print(f"cv2_array must be a numpy array.")
    if not isinstance(polygon_wkt, str):
        print(f"polygon_wkt must be a string.")
    if not isinstance(config.epsg_code, int):
        print(f"epsg_code must be an integer.")
    ...
    if cv2_array.dtype == np.uint8:
        ...
        print(f"Unsupported data type: {str(cv2_array.dtype)}")
...
def create_mosaic(directory, output_base_path, mosaic_size=(4000, 3500), border_size=1, border_color='black'):
    images_paths = [p for p in Path(directory).iterdir() if p.suffix.lower() in IMAGE_EXTENSIONS]
    num_images = len(images_paths)
    if num_images == 0:
        return
    print(images_paths, num_images)
    ...
    mosaic_image = PILImage.new('RGB', mosaic_size)
    ...
    for img_path in images_paths:
        ...
        img_resized = PILImageOps.expand(img_resized, border=border_size, fill=border_color)
    ...
    # Adjusting the output path to ensure it points to the correct subdirectory and file
    output_path = output_base_path / "mosaic.jpg"

    mosaic_image.save(output_path, format='JPEG')


def create_mosaic_orig(directory, output_base_path, mosaic_size=(400, 350), border_size=1, border_color='black'):
    images_paths = [p for p in Path(directory).iterdir() if p.suffix.lower() in IMAGE_EXTENSIONS]
    num_images = len(images_paths)
    if num_images == 0:
        return
